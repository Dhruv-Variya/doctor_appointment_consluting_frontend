...
function Verifierreg() {
    ...
    const { name, email, password } = verifierdetails;
    const handleInput = (e) => {
    ...
    const submit = async e => {
        if (verifierdetails.name == "" || verifierdetails.email == "" || verifierdetails.password == "") {
            ...
            e.preventDefault();
            await axios.post(`${process.env.REACT_APP_BASE_URL}/verifierreg`, verifierdetails).then((res) => {
                console.log(res);
                if (res.data.message == "ok") {
                    toast.success("registration Successful");
                    toast.info("Verifier added successfully");
                    // navigator();
                    var i = 0;
                    var count = 0;
                    for (i = 0; i < 100000; i++) {
